...
- **`PLAN_FILE_PATH`** = construct after PLAN_DATE and PLAN_SLUG are set using Bash to expand `~`:
  ...
  ```
  Capture the output (an absolute path like `~/.minime/workspace/reference/plans/<PLAN_DATE>-<PLAN_SLUG>.md`) as `PLAN_FILE_PATH`. Use this absolute path everywhere.
...
- `PLAN_FILE_PATH` = the absolute path captured in Phase 0.4 (e.g., `~/.minime/workspace/reference/plans/<PLAN_DATE>-<PLAN_SLUG>.md`). Substitute the actual absolute path into all validator prompts.
